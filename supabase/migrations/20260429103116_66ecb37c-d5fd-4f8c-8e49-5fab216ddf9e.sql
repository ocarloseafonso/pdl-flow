...
-- ============ PROMPT TEMPLATES (editáveis pelo usuário) ============
CREATE TABLE public.prompt_templates (
  id TEXT PRIMARY KEY,
  user_id UUID, -- NULL = template global padrão
  title TEXT NOT NULL,
...
('site', 'Prompt 1 — Personalização do Site', 'Você é um especialista em desenvolvimento web e SEO local. Vou te fornecer os dados de um cliente. Sua tarefa é personalizar completamente este template de site para ele. Leia todos os arquivos do projeto antes de começar. Não invente informações — use apenas o que está nos dados abaixo. Se algum dado não foi fornecido, mantenha o placeholder original e me avise ao final.
...
Bairros/regiões atendidas: {{areas}}
...
Serviço principal: {{main_service}}
Outros serviços: {{other_services}}
Problema que resolve: {{problem_solved}}
Público-alvo: {{audience}}
Diferencial: {{differentiator}}
O que os clientes elogiam: {{praises}}
Dúvidas frequentes dos clientes: {{faq}}
O que não atende: {{restrictions}}
Trabalha sozinho ou com equipe: {{team}}
Atendimentos por dia: {{daily_capacity}}
Duração média do atendimento: {{avg_duration}}
Agendamento ou ordem de chegada: {{scheduling}}
Formas de pagamento: {{payment_methods}}
Promoções ou ofertas: {{promotions}}
Estacionamento: {{parking}}
Acessibilidade: {{accessibility}}
Banheiro para clientes: {{restroom}}
Wi-fi: {{wifi}}
Bom para crianças: {{kid_friendly}}
Slogan: {{slogan}}
Cores da marca: {{brand_colors}}

TAREFAS — execute nesta ordem:

1. HTML — todos os arquivos
- Substitua todas as ocorrências do nome antigo do template pelo nome da empresa
- Atualize a seção hero com o slogan do cliente
- Atualize a seção "Sobre" com a bio, data de abertura e história do responsável
- Atualize os cards de serviços com os serviços reais do cliente
- Atualize horários, formas de pagamento, formas de atendimento
- Atualize todos os links de redes sociais
- Atualize o número do WhatsApp em todos os botões de contato
- Na seção de FAQ, use as dúvidas frequentes do cliente
- Atualize o rodapé com endereço, cidade, telefone e e-mail
- Nas páginas de blog, atualize apenas título e meta description (conteúdo será gerado separadamente)

2. SEO — tags <head> de cada página
- <title>: "Nome da empresa — Serviço principal — Cidade"
- <meta name="description">: até 155 caracteres com cidade, nicho e diferencial
- <meta name="keywords">: nicho + cidade + bairros + serviços principais
- <link rel="canonical"> em cada página
- Schema JSON-LD LocalBusiness no <head> da home

3. CSS
- Se cores da marca foram fornecidas, substitua as variáveis principais mantendo contraste

4. Alt text de imagens
- Todos os alt devem descrever serviço + cidade
...
('blog_pauta', 'Prompt 2 — Pauta do Blog (9 artigos)', 'Você é um especialista em SEO local e marketing de conteúdo. Com base nos dados do cliente abaixo, defina 9 temas de artigos para o blog do site. O objetivo é que cada artigo seja encontrado no Google por pessoas que buscam o serviço na cidade do cliente, e que sirva de referência para IAs quando alguém perguntar pelo melhor profissional do nicho na cidade.
...
CRITÉRIOS:
- Cada tema responde uma dúvida real do público antes de contratar
- Cidade ou bairro no título em pelo menos 6 dos 9 artigos
- Nenhum título genérico — citar serviço/nicho de forma específica
- Variar formatos: 2 guias completos, 2 comparativos, 2 listas práticas, 2 FAQ, 1 autoridade
- Progressão: do desconhecido ao pronto para contratar

ENTREGUE uma tabela em formato CSV (separado por |):
posicao | titulo | palavra_chave | intencao | formato | prioridade

Após a tabela, liste os 3 artigos prioritários e justifique.'),

('blog_artigo', 'Prompt 3 — Geração de Artigo (3000 palavras)', 'Você é um redator especialista em SEO local e conteúdo de autoridade. Vou te passar os dados do cliente e o tema do artigo. Escreva um artigo completo, mínimo 2800 palavras, que funcione como conteúdo útil, página de conversão e referência para buscadores e IAs.
...
2. Introdução (150–200 palavras) — problema real, promessa de resolver, menciona cidade
3. O que é / Como funciona (300–400 palavras)
4. Por que isso importa para quem mora em {{city_state}} (200–300 palavras) — cita bairros
5. Desenvolvimento principal (800–1000 palavras) — H2/H3, listas onde fizer sentido
6. Erros comuns / O que evitar (200–300 palavras)
7. Como escolher um bom profissional em {{city_state}} (200–300 palavras)
8. FAQ (300–400 palavras) — mínimo 5 perguntas, formato schema-ready
9. Conclusão + CTA (150–200 palavras) — nome da empresa, WhatsApp e Instagram

REGRAS:
- Tom direto, humano, sem enrolação
- Cidade mencionada pelo menos 8 vezes naturalmente
- Nome da empresa pelo menos 4 vezes
- Não usar: "incrível", "revolucionário", "único no mercado", "melhor do Brasil"
- Não fazer promessas não confirmadas
- Cada H2 entre 150 e 300 palavras

Ao final, entregue: título SEO (até 60 chars), meta description (até 155 chars), 5 sugestões de alt text para imagens.');
...
-- phases / task_templates / holidays: leitura pública (são catálogos)
...
-- clients: dono gerencia tudo + leitura pública via briefing_token (para o formulário público)
...
-- prompt_templates: leitura por todos os autenticados (são padrão), edição livre para dono
...
-- ============ TRIGGER: criar tasks automaticamente quando cliente é criado ============
